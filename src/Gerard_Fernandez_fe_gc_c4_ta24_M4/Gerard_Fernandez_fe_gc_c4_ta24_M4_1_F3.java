...
public class Gerard_Fernandez_fe_gc_c4_ta24_M4_1_F3 {
	...
	public static void main(String[] args) {
        ...
        while (continueOrdering) {
            ...
            //Condicional para comprobar si continua pidiendo platos
            if (continueOption == 0) {
        ...
        for (int i = 0; i < selectedPlate.size(); i++) {
        	for (int j = 0; j < menu.length; j++) {
				if(selectedPlate.get(i).equalsIgnoreCase(menu[j])) {
					count++;
					totalPrice += prices[j];
				}
			}
        ...
        //el count lleva la cuenta de los platos coincidentes si no son iguales significa que no existen algunos de los platos introducidos
        if(count!=selectedPlate.size()) {
